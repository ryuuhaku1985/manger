...
/**
 * Created by zhangshuo on 19/1/9.
 */
public class DateUtil {
    ...
    public static final String DATE_FORMAT_M_DOT_D = "M.d";
     ...
     * @return
     */
    public static long getDiffHours(Timestamp from, Timestamp to) {
...
//                DateUtil.DATE_FORMAT_YYYYMMDDHH));
    ...
    public static Timestamp getEndOfToday() {
        Calendar cal = Calendar.getInstance();
        SimpleDateFormat form = new SimpleDateFormat("yyyy-MM-dd");
        String dateStr = form.format(cal.getTime()) + " 23:59:59.999";
        Date date = null;
        try {
    ...
    public static String getOnlyMonth(Timestamp time) {
...
//		System.err.println("dayOfWeek:" + dayOfWeek + ",diffdays：" + diffdays);
    ...
    //两个时间一分钟之内显示xx秒,一个小时内显示xx分钟xx秒,一个小时后显示xx小时
    public static String parseTwoTime2Str(Timestamp time1, Timestamp time2) {
        long diffSeconds = getDiffSeconds(time1, time2);
        StringBuffer str = new StringBuffer();
        if (diffSeconds < 60) {
            str.append(getDiffSeconds(time1, time2)).append("秒");
        } else if (diffSeconds < 60 * 60) {
            long diffMinutes = getDiffMinutes(time1, time2);
            long seconds = diffSeconds - diffMinutes * 60;
            str.append(diffMinutes).append("分钟");
            if (seconds > 0) {
                str.append(seconds).append("秒");
            }
        } else {
            long diffHours = getDiffHours(time1, time2);
            long minutes = getDiffMinutes(time1, time2) - diffHours * 60;
            str.append(diffHours).append("小时");
            if (minutes > 0) {
